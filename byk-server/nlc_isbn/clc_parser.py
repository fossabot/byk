...
# * Modified by Andrija Junzki in 2025 to:
# - Converted hard-coded dictionary to gzip-compressed JSON for better performance.
# - Implemented Gzip/JSON loading logic to replace static variable access.
# - Integrated into the ByK Service.
...
CLC_JSON_PATH = CURRENT_DIR / 'clc.json.gz'
...
class Parser:
    ...
    @staticmethod
    def load_tree_json_from_file(file_path: str = None):
        if not file_path:
            file_path = CLC_JSON_PATH
    ...
    def __init__(self):
        ...
        tree = self.load_tree_json_from_file()
...
class LazyParser(LazyObject):
    def _setup(self):
        self._wrapped = Parser.get_instance()


clc_parser = LazyParser()
